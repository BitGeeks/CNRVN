#include <a_samp>
/*
		 Only for www.PLAYSAMP.pro
*/

#define HeadText		"Trinh duyet hanh dong by Le_Song_Vi))"
#define MAX_ROWS		11
#define MAX_LIB			130
...
public OnFilterScriptInit()
{
	...
	LTD[10] = TextDrawCreate(550.888000, 414.222167, "CAI DAT");
	...
	LTD[14] = TextDrawCreate(585.776794, 233.740615, "THOAT");
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
	if(clickedid == LTD[9])
	{
		switch(save_Menu)
		{
			case 1:
			{
				...
				TextDrawSetString(LTD[14], "Quay lai");
			}
			case 2:
			{
	...
	else if(clickedid == LTD[14])
	{
		switch(save_Menu)
		{
			...
			case 2:
			{
				...
				save_Name = INVALID_IDX;
				UpdateLine(1);
				TextDrawSetString(LTD[14], "THOAT");
				SelectTD(LTD[9], false, "----");
	...
	{
		ShowPlayerDialog(EditID, 19111, DIALOG_STYLE_INPUT, "Luu hanh dong", " {c4c4c4}Ghi chu hanh dong... (tuy chon)", "Tiep theo", "Dong");
	}
	else
	{
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		case 19111: if(response)
		{
			...
			fclose(File);
			format(string, 120, "Trinh duyet {ffffff}da duoc luu:  %s", animname);
		...
		case 19112: if(response)
		{
			switch(listitem)
			{
				case 0: ShowPlayerDialog(EditID, 19113, DIALOG_STYLE_INPUT, "Toc do", "{c4c4c4}Chi dinh toc do. Mac dinh: 4.1", "Tiep", "Quay lai");
				case 1: a_Cycle ^= true, ShowSettingDialog();
				case 2: a_LockPos ^= true, ShowSettingDialog();
				case 3: a_Freeze ^= true, ShowSettingDialog();
				case 4: ShowPlayerDialog(EditID, 19114, DIALOG_STYLE_INPUT, "Thoi gian", "{c4c4c4}Chi dinh thoi gian. Mac dinh: 0", "Tiep", "Quay lai");
		...
		case 19113:
		{
			if(response)
			{
				...
				if(speed < 0.1000 || speed > 4.10000) return ShowPlayerDialog(EditID, 19113, DIALOG_STYLE_INPUT, "Toc do", "{ff0000}Sai gia tri!     {c4c4c4}0.1 — 4.1\nChi dinh toc do. Mac dinh: 4.1", "Tiep", "Quay lai");
				a_Speed = speed + 0.001;
			}
			ShowSettingDialog();
		}
		case 19114:
		{
			if(response)
			{
				...
				if(time < 0 || time > 255) return ShowPlayerDialog(EditID, 19114, DIALOG_STYLE_INPUT, "Thoi gian", "{ff0000}Sai gia tri!\n{c4c4c4}Chi dinh thoi gian. Mac dinh: 0", "Tiep", "Quay lai");
				a_Time = time;
			}
			ShowSettingDialog();
		}
...
{
	if(!strcmp("/animlist", cmdtext, true, 5)) return cmd_anim(playerid);
	return false;
}
...
{
	new string[260];
	format(string, 260,"{c4c4c4}Toc do\t\t{bf8e13}| {ffce63}%.1f\n\
						{c4c4c4}Xoay vong\t\t{bf8e13}| %s\n\
						{c4c4c4}Khoa vi tri\t{bf8e13}| %s\n\
						{c4c4c4}Dong bang\t\t{bf8e13}| %s\n\
						{c4c4c4}Thoi gian\t\t{bf8e13}| {ffce63}%i\n\
	...
	ShowPlayerDialog(EditID, 19112, DIALOG_STYLE_LIST, "Cai dat", string, "Chon", "Dong");
}
