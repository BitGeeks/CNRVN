...
CMD:animlist(playerid, params[])// 24/02/2014
{
    ...
    ShowPlayerDialog(playerid, DIALOG_SHOW_INFO, DIALOG_STYLE_MSGBOX, "{FFFFFF}Lenh hanh dong", str, "Ok", "Huy bo");
	return 1;
}
...
CMD:surrender(playerid,params[])
 	{
		if(GetPlayerState(playerid) == PLAYER_STATE_ONFOOT && GetPVarInt(playerid, "Da bi thuong") == 0)
		{
		    SetPlayerSpecialAction(playerid,SPECIAL_ACTION_HANDSUP);
  	  		return 1;
		}
		else return SendClientMessage(playerid, COLOR_GRAD2, "Ban khong the su dung lenh nay ngay bay gio.");
	}
...
CMD:cheer(playerid,params[])
 ...
 CMD:urinate(playerid,params[]){
	    ...
	    if(GetPVarInt(playerid, "Injured") == 1) return SendClientMessage(playerid, COLOR_GRAD2, "Ban khong the su dung lenh nay ngay bay gio.");
...
CMD:lranim(playerid, params[])
{
	if(IsInLowRider(playerid))
	{
		...
		{
		    SendClientMessage(playerid, COLOR_GRAD2, "  Ban khong the la tai xe cua phuong tien!");
		    return 1;
		}
		switch(choice)
		{
